...
#define BEEP_OFF systemBeep(0)
#define BEEP_ON systemBeep(1)
...
pidConfig_t pidConfig_System; pidConfig_t pidConfig_Copy;
extern const pgRegistry_t pidConfig_Registry;
const pgRegistry_t pidConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = {
  .pgn = 504 | (2 << 12),
  .size = sizeof(pidConfig_t) | PGR_SIZE_SYSTEM_FLAG,
  .address = (uint8_t*)&pidConfig_System,
  .copy = (uint8_t*)&pidConfig_Copy, .ptr = 0,
  .reset = {.ptr = (void*)&pgResetTemplate_pidConfig}, };

const pidConfig_t pgResetTemplate_pidConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .pid_process_denom = 4,
  .runaway_takeoff_prevention = 1,
...
extern void pgResetFn_pidProfiles(pidProfile_t *);
pidProfile_t pidProfiles_SystemArray[3];
pidProfile_t pidProfiles_CopyArray[3];
extern const pgRegistry_t pidProfiles_Registry;
const pgRegistry_t pidProfiles_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = {
  .pgn = 14 | (2 << 12),
  .size = (sizeof(pidProfile_t) * 3) | PGR_SIZE_SYSTEM_FLAG,
  .address = (uint8_t*)&pidProfiles_SystemArray,
...
void resetPidProfile(pidProfile_t *pidProfile)
{
    static const pidProfile_t _reset_template_133 = {
      .pid = { [PID_ROLL] = { 40, 40, 30 },
	       [PID_PITCH] = { 58, 50, 35 },
	       [PID_YAW] = { 70, 45, 20 },
	       [PID_ALT] = { 50, 0, 0 },
	       [PID_POS] = { 15, 0, 0 },
      ...
      .crash_recovery = PID_CRASH_RECOVERY_OFF,
      .horizon_tilt_effect = 75,
      .horizon_tilt_expert_mode = 0,
      .crash_limit_yaw = 200,
      .itermLimit = 150 };
    memcpy((pidProfile), &_reset_template_133, sizeof(*(pidProfile)));
     ;
}
...
static float calcHorizonLevelStrength(void)
{
    ...
    float horizonLevelStrength =
      1.0f - __extension__ ({ __typeof__ (getRcDeflectionAbs(FD_ROLL)) _a = (getRcDeflectionAbs(FD_ROLL));
	  __typeof__ (getRcDeflectionAbs(FD_PITCH)) _b = (getRcDeflectionAbs(FD_PITCH));
	  _a > _b ? _a : _b; });
    ...
    const float currentInclination =
	  ...
	  __typeof__ (__extension__ ({
		...
		_x > 0 ? _x : -_x; })) _a =
	    (__extension__ ({ __typeof__ (attitude.values.roll) _x =
		  (attitude.values.roll); _x > 0 ? _x : -_x; }));
	  __typeof__ (__extension__ ({ __typeof__ (attitude.values.pitch) _x = (attitude.values.pitch);
		_x > 0 ? _x : -_x; })) _b = (__extension__ ({ __typeof__ (attitude.values.pitch) _x =
			(attitude.values.pitch); _x > 0 ? _x : -_x; })); _a > _b ? _a : _b; }) / 10.0f;
...
void pidController(const pidProfile_t *pidProfile, const rollAndPitchTrims_t *angleTrim, timeUs_t currentTimeUs)
{
    ...
    const float dynCi =
      __extension__ ({
	  __typeof__ ((1.0f - motorMixRange) * ITermWindupPointInv) _a = ((1.0f - motorMixRange) * ITermWindupPointInv);
	  __typeof__ (1.0f) _b = (1.0f); _a < _b ? _a : _b; }) * dT * itermAccelerator;
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; axis++) {
        ...
        if (inCrashRecoveryMode && cmpTimeUs(currentTimeUs, crashDetectedAtUs) > crashTimeDelayUs) {
		    ...
		    // && ABS(gyro.gyroADCf[FD_YAW]) < crashRecoveryRate)) {
                       && __extension__ ({
			   __typeof__ (gyro.gyroADCf[FD_ROLL]) _x = (gyro.gyroADCf[FD_ROLL]);
			   _x > 0 ? _x : -_x; }) < crashRecoveryRate
                       && __extension__ ({
			   __typeof__ (gyro.gyroADCf[FD_PITCH]) _x = (gyro.gyroADCf[FD_PITCH]);
			   _x > 0 ? _x : -_x; }) < crashRecoveryRate
                       && __extension__ ({
			   __typeof__ (gyro.gyroADCf[FD_YAW]) _x = (gyro.gyroADCf[FD_YAW]);
			   _x > 0 ? _x : -_x; }) < crashRecoveryRate)) {
                if (sensors(SENSOR_ACC)) {
		  ...
		  // && ABS(attitude.raw[FD_PITCH] - angleTrim->raw[FD_PITCH]) < crashRecoveryAngleDeciDegrees) {
                    if (__extension__ ({
			  __typeof__ (attitude.raw[FD_ROLL] - angleTrim->raw[FD_ROLL]) _x = (attitude.raw[FD_ROLL] - angleTrim->raw[FD_ROLL]);
			  _x > 0 ? _x : -_x; }) < crashRecoveryAngleDeciDegrees
                       && __extension__ ({
			   __typeof__ (attitude.raw[FD_PITCH] - angleTrim->raw[FD_PITCH]) _x = (attitude.raw[FD_PITCH] - angleTrim->raw[FD_PITCH]);
			   _x > 0 ? _x : -_x; }) < crashRecoveryAngleDeciDegrees) {
        ...
        if (outputSaturated == false
	    || __extension__ ({ __typeof__ (ITermNew) _x = (ITermNew);
		_x > 0 ? _x : -_x; }) <
	    __extension__ ({ __typeof__ (ITerm) _x = (ITerm); _x > 0 ? _x : -_x; })) {
        ...
        if (axis != FD_YAW) {
            ...
            const float rD = dynCd * __extension__ ({ __typeof__ (getRcDeflectionAbs(axis) * relaxFactor) _a = (getRcDeflectionAbs(axis) * relaxFactor);
		__typeof__ (1.0f) _b = (1.0f); _a < _b ? _a : _b; }) * currentPidSetpoint - gyroRateFiltered;
	      ...
	      // if (ARMING_FLAG(ARMED)) {
                if ((armingFlags & (ARMED))) {
                    if (motorMixRange >= 1.0f && !inCrashRecoveryMode
                        ...
                        && __extension__ ({ __typeof__ (delta) _x = (delta);
			    _x > 0 ? _x : -_x; }) > crashDtermThreshold
                        && __extension__ ({ __typeof__ (errorRate) _x = (errorRate);
			    _x > 0 ? _x : -_x; }) > crashGyroThreshold
                        && __extension__ ({ __typeof__ (getSetpointRate(axis)) _x = (getSetpointRate(axis));
			    _x > 0 ? _x : -_x; }) < crashSetpointThreshold) {
