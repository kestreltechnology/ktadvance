...
static int filter_mac(int family, char *addrp, char *mac, size_t maclen, void *parmv)
{
  ...
  for (arp = arps; arp; arp = arp->next)
   ...
   if (!
       (__extension__ ({
	   const struct in6_addr *__a = (const struct in6_addr *) (&arp->addr.addr.addr6);
	   const struct in6_addr *__b = (const struct in6_addr *) ((struct in6_addr *)addrp);
  ...
  if (difftime(now, last) < INTERVAL)
      ...
      for (arp = arps; arp; arp = arp->next)
	{
	  ...
	  if (arp->family == AF_INET6 && 
	      !(__extension__ ({
		    const struct in6_addr *__a = (const struct in6_addr *) (&arp->addr.addr.addr6);
		    const struct in6_addr *__b = (const struct in6_addr *) (&addr->in6.sin6_addr);
		    __a->__in6_u.__u6_addr32[0] == __b->__in6_u.__u6_addr32[0]
		      && __a->__in6_u.__u6_addr32[1] == __b->__in6_u.__u6_addr32[1]
		      && __a->__in6_u.__u6_addr32[2] == __b->__in6_u.__u6_addr32[2]
		      && __a->__in6_u.__u6_addr32[3] == __b->__in6_u.__u6_addr32[3]; })))
...
int do_arp_script_run(void)
{
  ...
  if (old)
    {
      if ((((53) < 32) ?
	   dnsmasq_daemon->options & (1u << (53)) :
	   dnsmasq_daemon->options2 & (1u << ((53) - 32))))
	queue_arp(7, old->hwaddr, old->hwlen, old->family, &old->addr);
