  ...
  for (context = daemon->dhcp6; context; context = context->next) 
	...
	*/
	...
	for (up = &old, slaac = old; slaac; slaac = slaac->next)
	  {
	    if ((__extension__ ({
		    const struct in6_addr *__a = (const struct in6_addr *) (&addr);
		    const struct in6_addr *__b = (const struct in6_addr *) (&slaac->addr );
...
void slaac_ping_reply(struct in6_addr *sender,
	      unsigned char *packet, char *interface, struct dhcp_lease *leases)
{
  struct dhcp_lease *lease;
  struct slaac_address *slaac;
  ...
  if (ping->identifier == ping_id)
	...
	if (slaac->backoff != 0 && 
	      (__extension__ ({
	    const struct in6_addr *__a = (const struct in6_addr *) (sender);
	    const struct in6_addr *__b = (const struct in6_addr *) (&slaac->addr);
	    __a->__in6_u.__u6_addr32[0] == __b->__in6_u.__u6_addr32[0]
	      && __a->__in6_u.__u6_addr32[1] == __b->__in6_u.__u6_addr32[1]
	      && __a->__in6_u.__u6_addr32[2] == __b->__in6_u.__u6_addr32[2]
	...
	// if (slaac->backoff != 0 && IN6_ARE_ADDR_EQUAL(sender, &slaac->addr))
	  {
	    ...
	    if (!(((43) < 32) ?
	      dnsmasq_daemon->options & (1u << (43)) :
	      dnsmasq_daemon->options2 & (1u << ((43) - 32))))
