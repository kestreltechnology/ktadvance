  ...
  while (1)
      ...
      if (rc == -1)
	{
	  ...
	  if (errno == EINVAL || errno == ENOENT)
	    ...
	    die(("cannot access path %s: %s"), path, EC_MISC);
	}
      else if (rc < size-1)
	{
	  char *d;
...
void inotify_dnsmasq_init()
{
  ...
  if (daemon->inotifyfd == -1)
    die(("failed to create inotify: %s"), NULL, EC_MISC);

  if ((((8) < 32)
       ? dnsmasq_daemon->options & (1u << (8))
       : dnsmasq_daemon->options2 & (1u << ((8) - 32))))
  ...
  for (res = daemon->resolv_files; res; res = res->next)
	...
	{
	  if (links-- == 0)
	    die(("too many symlinks following %s"), res->name, EC_MISC);
	  free(path);
	  path = new_path;
	}
      ...
      if ((d = strrchr(path, '/')))
	{
	  *d = 0; /* make path just directory */
	  res->wd = inotify_add_watchb(daemon->inotifyfd, path, IN_CLOSE_WRITE | IN_MOVED_TO);

	  res->file = d+1; /* pointer to filename */
	  *d = '/';
	  
	  if (res->wd == -1 && errno == ENOENT)
	    die(("directory %s for resolv-file is missing, cannot poll"), res->name, EC_MISC);
	}	  
	 
      if (res->wd == -1)
	die(("failed to create inotify for %s: %s"), res->name, EC_MISC);
...
void set_dynamic_inotify(int flag, int total_size, struct crec **rhash, int revhashsz)
{
  ...
  for (ah = daemon->dynamic_dirs; ah; ah = ah->next)
	...
	continue;
      ...
      if (stat(ah->fname, &buf) == -1 || !(((((buf.st_mode)) & 0170000) == (0040000))))
	{
	  my_syslog(LOG_ERR, ("bad dynamic directory %s: %s"), 
	 ...
	 {
	   my_syslog(LOG_ERR, ("failed to create inotify for %s: %s"),
		     ah->fname, strerror(errno));
	   continue;
	 }
	   ...
	   if ((path = whine_malloc(lendir + lenfile + 2)))
	     {
	       ...
	       if (stat(path, &buf) != -1 && ((((buf.st_mode)) & 0170000) == (0100000)))		       {
		...
		if ((path = whine_malloc(lendir + in->len + 2)))
		  {
		    ...
		    my_syslog(LOG_INFO, ("inotify, new or changed file %s"), path);
